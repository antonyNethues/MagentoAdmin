...
open class SlideMenuController: UIViewController, UIGestureRecognizerDelegate {
    ...
    open func initView() {
      ...
      if leftViewController != nil {
        ...
        leftFrame.origin.y = 60
        leftFrame.size.height = leftFrame.size.height - leftFrame.origin.y
        ...
        view.insertSubview(leftContainerView, at: 3)
        addLeftGestures()
      }
      ...
      if rightViewController != nil {
        ...
        rightFrame.origin.y = 60
        rightFrame.size.height = rightFrame.size.height - rightFrame.origin.y
    ...
    open override var supportedInterfaceOrientations : UIInterfaceOrientationMask {
        ...
        //on landscape orientation change
        self.closeLeft()
        
        if leftViewController != nil {
            var rightFrame = leftContainerView.frame
            if self.view.frame.size.height > self.view.frame.size.width {
                rightFrame.origin.y = 60
            }
            else {
                rightFrame.origin.y = 40
                if self.view.frame.size.height == 320 {
                    rightFrame.origin.y = 27
                }
            }
            //print(self.navigationController?.navigationBar.frame.size.height)
            print("right frame", rightFrame.origin.y)
            rightFrame.size.height = self.view.frame.size.height - rightFrame.origin.y
            leftContainerView.frame = rightFrame
